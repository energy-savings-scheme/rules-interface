...
describe('Calculate F7 Eligibility.', () => {
  const urlPath: string = URLPath.F7_E;
  let dataExcel: DataExcel;
  let rowsData: Record<string, any>[];

  before(() => {
    cy.task<WorkBook>('getDataExcel', EXCEL_PATH).then((workbook) => {
      dataExcel = new DataExcel(workbook, SheetName.F7_E);
  ...
  it('Successfully calculate commercial motors eligibility with eligible or ineligible result. Row data 1', () => {
  ...
  it('Successfully calculate commercial motors eligibility with eligible or ineligible result. Row data 2', () => {
  ...
  // it('Successfully calculate commercial motors eligibility with eligible or ineligible result.', () => {
