...
describe('Calculate F16-G Eligibility.', () => {
  const urlPath: string = URLPath.F16_G_E;
  let dataExcel: DataExcel;
  let rowsData: Record<string, any>[];

  before(() => {
    cy.task<WorkBook>('getDataExcel', EXCEL_PATH).then((workbook) => {
      dataExcel = new DataExcel(workbook, SheetName.F16_G_E);
  ...
  it('Successfully calculate commercial gas water heater - heat pump eligibility with eligible or ineligible result. Row data 1', () => {
  ...
  it('Successfully calculate commercial gas water heater - heat pump eligibility with eligible or ineligible result. Row data 2', () => {
  ...
  // it('Successfully calculate commercial gas water heater - heat pump eligibility with eligible or ineligible result.', () => {
