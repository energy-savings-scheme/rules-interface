...
describe('Calculate F17 Eligibility.', () => {
  const urlPath: string = URLPath.F17_E;
  let dataExcel: DataExcel;
  let rowsData: Record<string, any>[];

  before(() => {
    cy.task<WorkBook>('getDataExcel', EXCEL_PATH).then((workbook) => {
      dataExcel = new DataExcel(workbook, SheetName.F17_E);
  ...
  it('Successfully calculate commercial new heat pump water-heater eligibility with eligible or ineligible result. Row data 1', () => {
  ...
  it('Successfully calculate commercial new heat pump water-heater eligibility with eligible or ineligible result. Row data 2', () => {
  ...
  // it('Successfully calculate commercial new heat pump water-heater eligibility with eligible or ineligible result.', () => {
