...
declare global {
  ...
  namespace Cypress {
    interface Chainable {
      ...
      calculate(
        input: CalculateFormInputType
      ): Chainable<void>;
      calculateEligibility(
        input: CalculateEligibilityFormInputType
      ): Chainable<void>;
      getDataExcel(
        filePath: string
      ): Chainable<WorkBook>;
      ...
      interceptPostcodeAPI(
        params: InterceptPostcodeAPIOptions
      ): Chainable<void>;
    }
  }
}
    ...
    Object.entries(data).forEach(([fieldName, value]) => {
      cy.get(`${formSelector}`,).then(($el) => {
        ...
        if (tag === "select") {
          ...
          // the options
          cy.get(`${formSelector} [data-ui-name="${fieldName}"]`).should(
            "be.exist"
          );
...
Cypress.Commands.add("checkCalculationResult", (fieldNames: string[], data: {[key: string]: any}) => {
  // const result: { [key: string]: string } = {};
  fieldNames.forEach((field) => {
    cy.get(`[data-ui-name="${field}"]`)
      .should("be.exist")
      .and("have.text", data[field])
  })
});
